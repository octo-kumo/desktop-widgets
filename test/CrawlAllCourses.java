...
import me.kumo.courses.Course;
import me.kumo.courses.CoursesCrawler;
...
import static me.kumo.Widgets.prefs;
...
public class CrawlAllCourses {
    public static void main(String[] args) throws IOException {
        ...
        ArrayList<Course> allCourses = CoursesCrawler.gson.fromJson(new FileReader("all_courses.json"), new TypeToken<ArrayList<Course>>() {
        }.getType());
        for (int i = 0; i < subjects.size(); i++) {
            ...
            for (int j = 0; j < courseList.size(); j++) {
                ...
                for (int k = 0; k < progress / 5; k++) bar.append("=");
                System.out.printf("\rCrawling %s %s ... [%-20s] %d%% (%d/%d)", course.subject, course.courseCode, bar, progress, j + 1, courseList.size());
                course.sections = CoursesCrawler.getCourseSections(minerva, 2025, CoursesCrawler.TERM.WINTER, subject, course.courseCode);
            }
            System.out.println();
