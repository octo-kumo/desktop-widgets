...
public class TimetableCrawler {
    ...
    public static Timetable getSchedule(Minerva minerva, int offset, boolean useCache) {
        ...
        if (useCache && TIMETABLES.containsKey(date))
        ...
        Timetable.Class[][] classes = transpose_clean(
                document.select("table[summary='This layout table is used to present the weekly course schedule.'] tr")
                        .stream().map(tr -> tr.select("th, td")).reduce(new ArrayList<List<Element>>(), (acc, row) -> {
                            if (!acc.isEmpty()
                                    && acc.get(acc.size() - 1).stream().anyMatch(td -> td.hasAttr("rowspan"))) {
                                AtomicInteger index = new AtomicInteger(0);
                                acc.get(acc.size() - 1).stream()
                                        .map(td -> new Tuple<>(index.getAndIncrement(),
                                                Integer.parseInt("0" + td.attr("rowspan")), td))
                                        .filter(it -> it.b() > 1)
                                        .forEach(it -> row.add(it.a(),
                                                new Element("td").attr("rowspan", String.valueOf(it.b() - 1))));
                            }
                            acc.add(row);
                            return acc;
        ...
        if (classes.length != 7 || Arrays.stream(classes).allMatch(d -> d.length == 0))
            classes = null;
    ...
    private static Timetable.Class[][] transpose_clean(Timetable.Class[][] array) {
        return range(0, array[0].length).parallel().mapToObj(r -> range(0, array.length).parallel()
                ...
                .map(a -> Arrays.stream(a).filter(Objects::nonNull)
                        .toArray(Timetable.Class[]::new))
                .skip(1).limit(7).toArray(Timetable.Class[][]::new);
    }
