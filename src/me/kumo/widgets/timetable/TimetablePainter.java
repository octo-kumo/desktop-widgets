...
import static me.kumo.widgets.utils.GraphicsUtils.drawStringAJ;
import static me.kumo.widgets.utils.GraphicsUtils.drawStringClipped;
...
public class TimetablePainter {
    ...
    public static int paint(Graphics2D g, Timetable schedule, Color[] theme, boolean strong, boolean align) {
        ...
        if (schedule == null || schedule.classes == null) {
            ...
            drawStringAJ(g, "No schedule", align ? W - 10 : 10, H - 10, BOTTOM, align ? RIGHT : LEFT,
                    strong ? bg : TRANSPARENT);
            return 20;
        }

        long now = LocalTime.now().toSecondOfDay();
        // LocalTime.ofSecondOfDay((LocalTime.now().toSecondOfDay() * 10000L) % (86400L
        // * 1000_000_000L));
        ...
        g.setFont(g.getFont().deriveFont(20f).deriveFont(strong ? Font.BOLD : Font.PLAIN));
        drawStringAJ(g,
                DayOfWeek.of(dday + 1) + (dday == day ? " " + LocalTime.ofSecondOfDay(now).format(DISPLAY)
                        : dday == (day + 1) % 7 ? " TMR" : "(IN " + (dday + 7 - day) % 7 + " DAYS)"),
                align ? W - 10 : 10, H - 10, BOTTOM, align ? RIGHT : LEFT, strong ? bg : TRANSPARENT);
        ...
        for (int i = 0; i < classes.length; i++) {
            ...
            drawStringAJ(g, LocalTime.ofSecondOfDay(classes[i].start()).format(DISPLAY), align ? W - 10 : 10, y, TOP,
                    align ? RIGHT : LEFT, strong ? bg : TRANSPARENT);
            drawStringAJ(g, classes[i].name(), align ? W - 75 : 75, y, TOP, align ? RIGHT : LEFT,
                    strong ? bg : TRANSPARENT);

            g.setFont(g.getFont().deriveFont(20f).deriveFont(Font.PLAIN));
            drawStringAJ(g, LocalTime.ofSecondOfDay(classes[i].end()).format(DISPLAY), align ? W - 10 : 10, y + H,
                    BOTTOM, align ? RIGHT : LEFT, strong ? bg : TRANSPARENT);

            g.setColor(new Color(128 << 24 | (c.getRGB() & 0x00FFFFFF), true));
            g.setFont(g.getFont().deriveFont(15f));
            drawStringAJ(g, classes[i].location(), align ? W - 75 : 75, y + H, BOTTOM, align ? RIGHT : LEFT,
                    strong ? bg : TRANSPARENT);
        }
        g.setColor(theme[0]);
    ...
    public static void paintWeek(Graphics2D g, Timetable schedule, Color[] theme, int w, int h, boolean weekends) {
        ...
        for (int m = 0; m < (maxH - minH) * 60; m += 30) {
            ...
            g.setColor(theme[1]);
            g.setFont(g.getFont().deriveFont(15f));
            drawStringAJ(g, LocalTime.of(minH, 0).plusMinutes(m).format(DISPLAY), left - 5, y, CENTER, RIGHT,
                    TRANSPARENT);
        }
        for (int day = 0; day < (weekends ? 7 : 5); day++) {
            g.setColor(theme[1]);
            g.setFont(g.getFont().deriveFont(20f));
            drawStringAJ(g, DayOfWeek.of(day + 1).toString(), day * colW + left + colW / 2f, top / 2f, CENTER, CENTER,
                    TRANSPARENT);
            ...
            for (Timetable.Class c : classes) {
                ...
                int topM = drawStringAJ(g, LocalTime.ofSecondOfDay(c.start()).format(DISPLAY), x + colW - 5, y + 5, TOP,
                        RIGHT, TRANSPARENT);
                int bottomM = drawStringAJ(g, LocalTime.ofSecondOfDay(c.end()).format(DISPLAY), x + colW - 5,
                        y + height - 5, BOTTOM, RIGHT, TRANSPARENT);

                drawStringClipped(g, c.name(), x + 5, y + 5, TOP, LEFT, TRANSPARENT, colW - 10 - topM);
                drawStringClipped(g, c.location(), x + 5, y + height - 5, BOTTOM, LEFT, TRANSPARENT,
                        colW - 10 - bottomM);
